...
run_model <- function(model_path, model_string, model_desc){
    ...
    label <- paste("post_est_", model_desc, sep="")
    ...
    label <- paste("exp_post_est_", model_desc, sep="")
    print(kbl(rr, booktabs = T, escape=T, caption=caption, label=label)) 
    
    # Model checking and convergence checking
    ...
    p_ints <- mcmc_intervals(exp(posterior), regex_pars = c("theta", "mu")) + interval_title
    print(p_ints)
}
